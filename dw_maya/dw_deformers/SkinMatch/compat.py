"""
compat.py - PySide2 / PySide6 compatibility layer for the SkinMatch tool.

Single source of truth for API differences between the two versions. All
SkinMatch files import Qt from here rather than directly, mirroring
DemBones/compat.py and DynForge/forge_cmds/compat.py.

This tool is expected to run in a production Maya that may still ship PySide2,
so the fallback is not optional here.
